...
> 3. **VALIDATE before live capital** — breach-bar-close fill timing + SL-gap optimism. An adversarial risk-lens (2026-06-16) rated this **REVISE**: the monitor fills at the bar close but runs next morning → real fill ≈ next open; on a +0.131R edge this gap could erode it to ~0. Concrete test: add a `next_open_fill` flag to the engine, re-run, confirm adj_R stays positive under next-open fills BEFORE any real capital. See [[Gotchas#Backtest reproducibility & live-exit divergence]].
> 5. **FINISH the advisory panel** — only the risk lens completed (session limit hit 2026-06-16); the **data-integrity, provenance, and refute lenses did NOT run**. They're the ones that answer "is the golden-file edge genuinely real or a flawed/overfit artifact?" Re-run them next session (esp. check the golden CSV for research-mode trade *overlap*, and whether reproducing one saved CSV proves anything). The live fix stays meanwhile (it replaced a known-broken exit), but treat the edge as **unconfirmed** until the panel finishes.
